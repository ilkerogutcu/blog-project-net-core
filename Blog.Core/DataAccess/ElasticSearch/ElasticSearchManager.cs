...
using Nest;
using Result = Blog.Core.Utilities.Results.Result;
...
namespace Blog.Core.DataAccess.ElasticSearch
{
    public class ElasticSearchManager : IElasticSearch
    {
        ...
        public async Task<long> GetCountAsync<T>(string indexName)
            where T : class
        {
            var elasticClient = GetElasticClient(indexName);
            var countResponse= await elasticClient.CountAsync<T>(x=>x.Index(indexName));
